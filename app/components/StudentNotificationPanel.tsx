...
interface Notification {
  _id: string;
  type: 'due_reminder' | 'overdue_alert' | 'borrow_confirmation' | 'return_confirmation' | 'fine_notice' | 'book_available' | 'reservation_expired';
  title: string;
  message: string;
  bookId?: string;
  bookTitle?: string;
...
export default function StudentNotificationPanel({ userId, isOpen, onClose }: StudentNotificationPanelProps) {
  const [notifications, setNotifications] = useState<Notification[]>([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [isLoading, setIsLoading] = useState(false);
  const [filter, setFilter] = useState<'all' | 'unread' | 'urgent' | 'books'>('all');
  ...
  useEffect(() => {
    if (isOpen && userId) {
      ...
      try {
        const cachedData = localStorage.getItem(`student_notifications_${userId}_${filter}`);
        if (cachedData) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (filter === 'urgent') {
          ...
          filteredNotifications = data.notifications.filter((n: Notification) => 
            n.type === 'due_reminder' || n.type === 'overdue_alert' || n.type === 'book_available'
          );
        }
        ...
        try {
          localStorage.setItem(`student_notifications_${userId}_${filter}`, JSON.stringify({
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[80vh] flex flex-col border border-gray-200">
        ...
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-blue-50 to-blue-100">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Student Notifications</h2>
            {unreadCount > 0 && (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p>No student notifications to display</p>
            </div>
          ) : (
